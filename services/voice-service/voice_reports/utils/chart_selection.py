...
def infer_chart_type(
    *,
    ...
    # Priority engine: correlation -> time_series -> distribution -> category comparison -> card
    if relationship_requested and _supports(ChartType.SCATTER):
        recommended = ChartType.SCATTER
    elif time_series_requested and _supports(ChartType.LINE):
        recommended = ChartType.LINE
    elif distribution_requested and _supports(ChartType.HISTOGRAM):
        recommended = ChartType.HISTOGRAM
    elif category_comparison_requested and has_category_value_shape and _supports(ChartType.BAR):
        recommended = ChartType.BAR
    elif single_value and _supports(ChartType.CARD):
        recommended = ChartType.CARD
    ...
    # Shape rules fallback: time+metric -> line, 2 numeric -> scatter, 1 numeric -> histogram, category+metric -> bar, single value -> card
    if not recommended:
        if _supports(ChartType.LINE):
            recommended = ChartType.LINE
        elif _supports(ChartType.SCATTER):
            recommended = ChartType.SCATTER
        elif _supports(ChartType.HISTOGRAM):
            recommended = ChartType.HISTOGRAM
        elif has_category_value_shape and _supports(ChartType.BAR):
            recommended = ChartType.BAR
        elif _supports(ChartType.BAR):
            recommended = ChartType.BAR
        elif _supports(ChartType.CARD):
            recommended = ChartType.CARD
        else:
