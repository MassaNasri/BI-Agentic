...
class SchemaInferenceEngine:
    ...
    def _infer_field_definition(
        ...
        # For schema evolution, we default to nullable=True for new fields
        # to maintain backward compatibility. If the field has nulls in the sample,
        # it's definitely nullable. If it doesn't, we still mark it as nullable
        # because we can't be certain from a sample that it should be required.
        nullable = True
    ...
    def _infer_constraints(self, stats: Dict[str, Any], data_type: DataType) -> List[Constraint]:
        ...
        if unique_count <= 10 and non_null_count > 0:
            # Only add enum constraint for hashable types
            try:
                ...
                # Skip enum constraint for unhashable types (lists, dicts)
                pass
        return constraints
...
class SchemaEvolutionDetector:
    ...
    def detect_evolution(
        ...
        # Preserve nullability of existing fields from current schema
        # Only new fields should default to nullable=True
        current_fields = {f.name: f for f in current_schema.fields}
        for field in inferred_schema.fields:
            if field.name in current_fields:
                # Preserve original nullability for existing fields
                field.nullable = current_fields[field.name].nullable
