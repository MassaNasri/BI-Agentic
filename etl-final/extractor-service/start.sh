...
while retry_count < max_retries:
    try:
        print(f'[EXTRACTOR] Attempting to start listener (attempt {retry_count + 1}/{max_retries})...')
        ...
        if retry_count < max_retries:
            print(f'[EXTRACTOR] Retrying in 10 seconds...')
            time.sleep(10)
        else:
            print(f'[EXTRACTOR] Max retries reached. Exiting.')
            sys.exit(1)
"
